 ...
 * @property {string} seperator Character separating options on a single line. @property {string} splitter Character dividing index and label for each option. */
export default class OptionsDisplay {
    // Constructor method: 
    // ------------------- //
    /** Initializes OptionsDisplay properties. 
     * @public @constructor @param {string} separator Option line separator. @param {string} splitter Character between number and option. @param {boolean} newLine Display on new line. */
    constructor(seperator="|",splitter=':',newline=true){this.new_line=TypeValidation.typeCheck(newline,Boolean),
        this.seperator=TypeValidation.typeCheck(seperator,String),
    ...
    setSeperator(seperator){this.seperator=TypeValidation.typeCheck(seperator,String)};
